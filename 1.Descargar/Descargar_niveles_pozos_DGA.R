...
descargar_niveles_DGA<-function(fecha_ini, fecha_fin, estaciones){
library(rvest)
library(writexl)
library(readxl)
library(dplyr)
...
getUrls2 <- function(year,cod_est){
  ...
  root3<-"_1_Nivel+de+Agua+%28m%29&accion=refresca&param=&tipo=ANO&fechaFinGrafico=15%2F11%2F2023&hora_fin=0&tiporep=S&period=rango&fechaInicioTabla=01%2F01%2F"
  #year<-"2023"
