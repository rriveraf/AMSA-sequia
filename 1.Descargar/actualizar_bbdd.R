...
actualizar_datos <- function(directorio_base, variable, fuente) {
  ...
  source(paste0(directorio_base, "/1.Descargar/Descargar.R"))
  ...
  if(length(archivo_mas_grande) != 0 && length(obtener_fechas_archivo(archivo_mas_grande)) != 0) {
    ...
    if(ano_fin == ano_actual && mes_fin == mes_ultimo) {
      print(paste("La base de datos de", variable, fuente, "está actualizada, no es necesario descargar datos"))
    } else { 
      ...
      resultado = concatenar_datos(data_nueva, list_fechas, ano_actual, mes_ultimo, variable, fuente, directorio_archivo)
    }
  } else {
