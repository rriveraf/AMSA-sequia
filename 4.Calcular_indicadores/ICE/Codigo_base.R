...
#ICE 3 meses
if(acumulacion ==3){
  q_3meses<-q_medio_mensual[13:nrow(q_medio_mensual),]#usamos mismo formato para las fechas y las estaciones, empezando desde el año 1991.
  q_3meses[,] <- NA #generamos valores de NA en donde pondremos los promedios acumulados
  
  #Promedio 3 meses
  for (col in c(1:ncol(q_3meses))){ 
    for (fila in 1:nrow(q_3meses)){
      datos_a_promediar <- q_medio_mensual[(fila+10):(fila+12),col] #creamos subconjunto con los 3 meses anteriores a la fecha a analizar + el mes actual
      datos_no_na <- sum(complete.cases(datos_a_promediar)) #contamos la cantidad de datos no NA en el subconjunto
      if (datos_no_na>=2){ #usamos un 82% de filtro (deben haber al menos 5 meses para obtener el promedio), cambiar por 6 si se desea datos en todos los meses
        a_promediar <- na.omit(datos_a_promediar)
      }
      else {
        a_promediar <- datos_a_promediar #en este caso al haber NA, el promedio queda como NA
      }
      q_3meses[fila,col] <- mean(datos_a_promediar)
    }
  }
  
  ICE3_MATRIZ <- q_3meses 
  ICE3_MATRIZ[,] <- NA
  
  #Coeficientes 3 meses
  Gamma_3meses <-subset(Carac_gamma,Acum==3)
  
  contador_estacion_ice3<-1
  for (i in colnames(ICE3_MATRIZ[,])){
    contador_estacion_gamma<-3
    for (j in colnames(Gamma_3meses[,3:ncol(Gamma_3meses)])){
      if(i==j){
        contador_fecha<-1
        for (fecha_dato in rownames(ICE3_MATRIZ)){
          k<-as.integer(substr(fecha_dato, 1, 2)) #lo queremos en formato número para llamarlo como posición
          Q<-q_3meses[contador_fecha,contador_estacion_ice3]
          ICE3_MATRIZ[contador_fecha,contador_estacion_ice3]<-calculo_ice(Q,Gamma_3meses[k,contador_estacion_gamma],Gamma_3meses[k+12,contador_estacion_gamma],Gamma_3meses[k+24,contador_estacion_gamma])
          contador_fecha<-contador_fecha+1
        }
      }
      contador_estacion_gamma<-contador_estacion_gamma+1
    }
    contador_estacion_ice3<-contador_estacion_ice3+1
  }
